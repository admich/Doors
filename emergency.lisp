                       ...
                       (xlib:event-case (dpy)
                         (:key-press (code window)
                                     (case (xlib:keycode->character dpy code 0)
                                       (#\q t)
                                       (#\t (clim-sys:make-process (lambda () (uiop:launch-program "xterm")) :name "emergency-thread") nil)
                                       (#\e (clim-sys:make-process (lambda () (uiop:launch-program "emacs")) :name "emergency-thread") nil)
                                       (#\k (clim-sys:make-process (lambda () (when-let ((wm *wm-application*)) (destroy-port (port wm)))) :name "emergency-thread") nil)
                                       (#\s (clim-sys:make-process (lambda () (doors:doors :new-process t)) :name "emergency-thread") nil)
                                       (t nil))))))
